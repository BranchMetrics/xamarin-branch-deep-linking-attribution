...
namespace IOSNativeBranch
{
	...
	// typedef void (^callbackForTracingRequests)(NSString * _Nullable, NSDictionary * _Nullable, NSDictionary * _Nullable, NSError * _Nullable, NSString * _Nullable);
	delegate void callbackForTracingRequests ([NullAllowed] string arg0, [NullAllowed] NSDictionary arg1, [NullAllowed] NSDictionary arg2, [NullAllowed] NSError arg3, [NullAllowed] string arg4);
...
// 	}
	...
	interface BNCPreferenceHelper
	{
		...
		[Export ("thirdPartyAPIsWaitTime")]
		double ThirdPartyAPIsWaitTime { get; set; }
		...
		[Export ("odmInfo")]
		string OdmInfo { get; set; }

		// @property (assign, nonatomic) NSTimeInterval odmInfoValidityWindow;
		[Export ("odmInfoValidityWindow")]
		double OdmInfoValidityWindow { get; set; }

		// @property (nonatomic, strong) NSDate * odmInfoInitDate;
		[Export ("odmInfoInitDate", ArgumentSemantic.Strong)]
		NSDate OdmInfoInitDate { get; set; }
		...
		// @property (nonatomic, strong) NSDate * urlLoadMs;
		[Export ("urlLoadMs", ArgumentSemantic.Strong)]
		NSDate UrlLoadMs { get; set; }
	...
	{
		// @property (readwrite, copy, nonatomic) NSString * requestUUID;
		[Export ("requestUUID")]
		string RequestUUID { get; set; }

		// @property (readwrite, copy, nonatomic) NSNumber * requestCreationTimeStamp;
		[Export ("requestCreationTimeStamp", ArgumentSemantic.Copy)]
		NSNumber RequestCreationTimeStamp { get; set; }
		...
		[Static]
		[Export ("generateRequestUUIDFromDate:")]
		string GenerateRequestUUIDFromDate (NSDate localDate);
	}
...
*/[Protocol]
	...
	interface BranchActivityItemProvider
	{
		...
		NativeHandle Constructor (NSDictionary @params, NSObject[] tags, string feature, string stage, string alias);
		...
		NativeHandle Constructor (NSDictionary @params, NSObject[] tags, string feature, string stage, string campaign, string alias, BranchActivityItemProviderDelegate @delegate);
	...
	interface Branch
	{
		...
		// -(void)initUserSessionAndCallCallback:(BOOL)callCallback sceneIdentifier:(NSString * _Nonnull)sceneIdentifier urlString:(NSString * _Nonnull)urlString reset:(BOOL)reset;
		[Export ("initUserSessionAndCallCallback:sceneIdentifier:urlString:reset:")]
		void InitUserSessionAndCallCallback (bool callCallback, string sceneIdentifier, string urlString, bool reset);
		...
		// -(void)enableLoggingAtLevel:(BranchLogLevel)logLevel withCallback:(BranchLogCallback _Nullable)callback __attribute__((deprecated("This API is deprecated. Please use the static version.")));
		// [Export ("enableLoggingAtLevel:withCallback:")]
		// void EnableLoggingAtLevel (BranchLogLevel logLevel, [NullAllowed] BranchLogCallback callback);
		...
		// +(void)setSafetrackAPIURL:(NSString * _Nonnull)url;
		[Static]
		[Export ("setSafetrackAPIURL:")]
		void SetSafetrackAPIURL (string url);

		// +(void)setCallbackForTracingRequests:(callbackForTracingRequests _Nonnull)callback;
		[Static]
		[Export ("setCallbackForTracingRequests:")]
		...
		[Static]
		[Export ("disableNextForeground")]
		void DisableNextForeground ();

		// +(void)disableNextForegroundForTimeInterval:(NSTimeInterval)timeout;
		[Static]
		[Export ("disableNextForegroundForTimeInterval:")]
		void DisableNextForegroundForTimeInterval (double timeout);
		...
		// +(void)setODMInfo:(NSString * _Nonnull)odmInfo andFirstOpenTimestamp:(NSDate * _Nonnull)firstOpenTimestamp;
		[Static]
		[Export ("setODMInfo:andFirstOpenTimestamp:")]
		void SetODMInfo (string odmInfo, NSDate firstOpenTimestamp);
		...
		string GetLongAppLinkURLWithParams (NSDictionary @params, [NullAllowed] string channel, NSObject[] tags, string feature, string stage, string alias);
	...
	[BaseType (typeof(NSObject))]
	[DisableDefaultCtor]
	interface BranchEvent : ISKProductsRequestDelegate
	{
		...
		[Export ("logEventWithTransaction:")]
	...
	interface BranchFileLogger
	{
		...
		[Static]
		[Export ("sharedInstance")]
		BranchFileLogger SharedInstance ();
		...
		[Export ("isLogFilePopulated")]
		bool IsLogFilePopulated { get; }

		// -(void)shareLogFileFromViewController:(UIViewController *)viewController;
		[Export ("shareLogFileFromViewController:")]
		void ShareLogFileFromViewController (UIViewController viewController);
	}
	...
	[DisableDefaultCtor]
	interface BranchPasteControl : IUIPasteConfigurationSupporting
	{
		// -(instancetype _Nonnull)initWithFrame:(CGRect)frame AndConfiguration:(UIPasteControlConfiguration * _Nullable)config __attribute__((objc_designated_initializer));
		// [Export ("initWithFrame:AndConfiguration:")]
		// [DesignatedInitializer]
		// NativeHandle Constructor (CGRect frame, [NullAllowed] UIPasteControlConfiguration config);
	}
	...
	interface BranchPluginSupport
	{
		...
		// +(void)setCDNBaseUrl:(NSString * _Nonnull)url;
		[Static]
		[Export ("setCDNBaseUrl:")]
	...
	{
		// @property (readwrite, nonatomic, strong) UIColor * _Nonnull codeColor;
		[Export ("codeColor", ArgumentSemantic.Strong)]
		UIColor CodeColor { get; set; }

		// @property (readwrite, nonatomic, strong) UIColor * _Nonnull backgroundColor;
		[Export ("backgroundColor", ArgumentSemantic.Strong)]
		UIColor BackgroundColor { get; set; }

		// @property (readwrite, copy, nonatomic) NSString * _Nonnull centerLogo;
		[Export ("centerLogo")]
		string CenterLogo { get; set; }

		// @property (readwrite, copy, nonatomic) NSNumber * _Nonnull width;
		[Export ("width", ArgumentSemantic.Copy)]
		NSNumber Width { get; set; }

		// @property (readwrite, copy, nonatomic) NSNumber * _Nonnull margin;
		[Export ("margin", ArgumentSemantic.Copy)]
		NSNumber Margin { get; set; }

		// @property (assign, readwrite, nonatomic) BranchQRCodeImageFormat imageFormat;
		[Export ("imageFormat", ArgumentSemantic.Assign)]
		BranchQRCodeImageFormat ImageFormat { get; set; }

		// -(void)getQRCodeAsData:(BranchUniversalObject * _Nullable)buo linkProperties:(BranchLinkProperties * _Nullable)lp completion:(void (^ _Nonnull)(NSData * _Nullable, NSError * _Nullable))completion;
		...
		// -(void)getQRCodeAsImage:(BranchUniversalObject * _Nullable)buo linkProperties:(BranchLinkProperties * _Nullable)lp completion:(void (^ _Nonnull)(UIImage * _Nullable, NSError * _Nullable))completion;
		[Export ("getQRCodeAsImage:linkProperties:completion:")]
		void GetQRCodeAsImage ([NullAllowed] BranchUniversalObject buo, [NullAllowed] BranchLinkProperties lp, Action<UIImage, NSError> completion);

		// -(void)showShareSheetWithQRCodeFromViewController:(UIViewController * _Nullable)viewController anchor:(id _Nullable)anchorViewOrButtonItem universalObject:(BranchUniversalObject * _Nullable)buo linkProperties:(BranchLinkProperties * _Nullable)lp completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
		[Export ("showShareSheetWithQRCodeFromViewController:anchor:universalObject:linkProperties:completion:")]
		void ShowShareSheetWithQRCodeFromViewController ([NullAllowed] UIViewController viewController, [NullAllowed] NSObject anchorViewOrButtonItem, [NullAllowed] BranchUniversalObject buo, [NullAllowed] BranchLinkProperties lp, Action<NSError> completion);
	}
	...
	// partial interface Constants
	// {
	// 	// extern double BranchSDKVersionNumber;
	// 	[Field ("BranchSDKVersionNumber", "__Internal")]
	// 	double BranchSDKVersionNumber { get; }

	// 	// extern const unsigned char[] BranchSDKVersionString;
	// 	[Field ("BranchSDKVersionString", "__Internal")]
	// 	byte[] BranchSDKVersionString { get; }
	// }
}
