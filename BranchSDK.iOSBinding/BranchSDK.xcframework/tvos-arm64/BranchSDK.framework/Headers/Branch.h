...
/**
 ...
 Configure SDK wait duration for third-party API responses (ODM info and Apple Attribution Token)
 
 @param waitTime - (NSTimeInterval) Number of seconds SDK will wait for third party APIs to finish. Default is 0.5 seconds (500ms).
 */
...
/**
 ...
 @param odmInfo - (NSString) The ODM event data (or aggregate conversion info) for the current app instance.
 @param firstOpenTimestamp - (NSDate) The date and time when the app was first opened after installation. This timestamp is used for conversion attribution timing and should match the value passed to Google's setFirstLaunchTime method.
 */
+ (void) setODMInfo:(NSString *) odmInfo andFirstOpenTimestamp:(NSDate *)firstOpenTimestamp;

/**
 
 Sets a custom Meta anon_id for the current user.
 
 @param anonID - (NSString) The custom Meta anon_id to set for the user. The Meta anon_id is a GUID generated by the Branch iOS SDK for Meta AEM.
 */
